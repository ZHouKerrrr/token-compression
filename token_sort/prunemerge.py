...
class PruneMergeTokenSorter(BaseTokenSorter):
    ...
    def _setup_module(self) -> None:
        ...
        self.use_topk = getattr(self.config, 'use_topk', True)
        self.topk_ratio = getattr(self.config, 'topk_ratio', 0.25)
        self.merge_config = getattr(self.config, 'merge_config', None)
