...
class RateLoss(nn.Module):
    ...
    def _log_ratio_loss(
        ...
        exact mode:
            loss = log(ratio / target)^2

        upper-bound mode:
            loss = max(0, log(ratio / target))^2

        upper-bound mode only penalizes ratio > target.
        """
        ...
        ratio = ratio.clamp_min(self.eps)
        target_tensor = target_tensor.clamp_min(self.eps)

        diff = torch.log(ratio) - torch.log(target_tensor)
    ...
    def forward(
        self, 
        keep_ratio: Dict[str, torch.Tensor] = None,
...
class SPARELoss(nn.Module):
    """Complete SPARE loss computation
...
# Helper function
def create_spare_loss(**kwargs) -> SPARELoss:
    """Create SPARE loss from config
    
    Args:
        config: SPARE configuration
        
    Returns:
        SPARELoss instance
    """
    return SPARELoss(**kwargs)
