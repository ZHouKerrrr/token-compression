...
class PATOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.lambda_rate = self.lambda_loss['lambda_rate']
        self.lambda_kd_logits = self.lambda_loss['lambda_kd_logits']
        self.lambda_distortion = self.lambda_loss['lambda_distortion']
        self.temperature_kd_logits = self.lambda_loss['temperature_kd_logits']
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if labels is not None:
            ...
            if self.teacher is not None:
                ...
                T = self.temperature_kd_logits  # 先试 2.0 或 4.0
                ...
                loss = self.lambda_distortion * distortion_loss + self.lambda_kd_logits * kd_loss + self.lambda_rate * rate_loss 
                if self.state.global_step % 50 == 0:
                    print_rank0(f"keep ratio: {keep_ratio}")
                    print_rank0(f"Distortion Loss: {self.lambda_distortion * distortion_loss}, KD Loss: {self.lambda_kd_logits * kd_loss}, Rate Loss: {self.lambda_rate * rate_loss[0]},  Loss: {loss[0]}")
            else:
                keep_ratio = aux_outputs["keep_ratio"]
                rate_loss = keep_ratio.mean(dim=0)
                distortion_loss = outputs["loss"]
